...
where  film.film_id not in (select film.film_id from film right join film_category on film.film_id= film_category.film_id) ;
select * from employees limit 6,5;
...
-- 5
insert into table_name values (1,4),(2,3),(3,3),(4,2),(5,5),(6,4);
select id,number ,row_number() over (order by number desc)from table_name  ;
```